...
type View = 'dashboard' | 'devices' | 'zones' | 'settings';
...
export function App() {
  ...
  const { getSettings, getDevices, getZones, getDiscordStatus, updateSettings } = useIPC();
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const [settingsData, devicesData, zonesData, discordStatus] = await Promise.all([
          getSettings(),
          getDevices(),
          getZones(),
          getDiscordStatus(),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatusCard
          label="Light Zones"
          value={`${zones.length}`}
          color={zones.length > 0 ? 'purple' : 'gray'}
        />
      </div>
